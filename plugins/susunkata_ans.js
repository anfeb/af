...
handler.before = async function (m) {
    ...
    if (/^nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.susunkata[id][1].jawaban))}`.trim(), '© @ANFEBN', 'SUSUN KATA', '.susunkata').then(() => {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
        ...
        await this.sendButton(m.chat, `*Benar!* +${this.susunkata[id][2]} XP`, '© @ANFEBN', 'SUSUN KATA', '.susunkata')
